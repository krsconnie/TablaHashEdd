...
> experiment_results_ID_lineal.log echo.

REM Imprimir el encabezado CSV en el archivo de salida final
echo numElementos;tiempo > experiment_output_lineal.csv
...
for /L %%c in (1,1,30) do (
    ...
    for %%n in (1000, 5000, 10000, 20000) do (
        .\test_ID.exe %%n >> experiment_results_ID_lineal.log
    )
)

REM Redirigir la salida del log al archivo CSV final
type experiment_results_ID_lineal.log >> experiment_output_lineal.csv

REM Eliminar los archivos .exe y .log
del test_ID.exe
del experiment_results_ID_lineal.log
