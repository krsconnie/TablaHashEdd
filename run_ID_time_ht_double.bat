...
> experiment_results_ID_ht_double.log echo.

REM Imprimir el encabezado CSV en el archivo de salida final
echo numElementos;tiempo > experiment_output_ht_double.csv
...
for /L %%c in (1,1,30) do (
    ...
    for %%n in (1000, 5000, 10000, 20000) do (
        .\test_ID.exe %%n >> experiment_results_ID_ht_double.log
    )
)

REM Redirigir la salida del log al archivo CSV final
type experiment_results_ID_ht_double.log >> experiment_output_ht_double.csv

REM Eliminar los archivos .exe y .log
del test_ID.exe
del experiment_results_ID_ht_double.log
